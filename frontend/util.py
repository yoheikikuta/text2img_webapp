...
def call_text2img(text: str) -> np.ndarray:
    ...
    return np.frombuffer(response.content, dtype=np.dtype('uint8')).reshape(64,64*5,3)
    # Update after implementing upsampler. return np.frombuffer(response.content).reshape(256,256*5,3)
